...
export default function BadgeCelebration() {
    const { newBadges, clearNewBadges } = useStore();
    ...
    const handleDismiss = () => {
        setActiveBadge(null);
        // Aspetta fine animazione uscita (se volessimo farla complessa), poi pulisci store
        setTimeout(() => {
            clearNewBadges();
        }, 300);
    };
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative z-10 w-full max-w-sm flex flex-col items-center animate-in zoom-in-90 duration-500">
                ...
                <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 ${colors.glow} blur-[60px] rounded-full animate-pulse mix-blend-multiply`} />

                {/* Trophy Circle */}
                <div className={`w-32 h-32 bg-gradient-to-br ${colors.from} ${colors.to} rounded-full flex items-center justify-center mb-8 shadow-[0_10px_40px_rgba(0,0,0,0.2)] border-4 ${colors.border} animate-bounce relative`}>
                    <span className="material-symbols-outlined text-white drop-shadow-[0_2px_10px_rgba(0,0,0,0.3)] text-[64px]" style={{ fontVariationSettings: "'FILL' 1" }}>trophy</span>
                ...
                <div className="text-center space-y-4 relative w-full">
                    <h2 className={`text-4xl font-black italic uppercase tracking-tighter drop-shadow-sm ${colors.text} animate-in slide-in-from-bottom-4 duration-700`}>
                        Badge Sbloccato!
                    </h2>

                    <div className="bg-white border border-slate-200 px-6 py-6 rounded-3xl shadow-xl animate-in slide-in-from-bottom-8 duration-700 delay-100 w-full">
                        ...
                        <div className={`inline-flex items-center gap-1.5 px-4 py-1.5 rounded-xl border ${colors.border} bg-slate-50 mb-4 shadow-inner`}>
                    ...
                    <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest animate-in fade-in delay-700 mt-4">
                        Tocca per chiudere
                    </p>
                </div>
